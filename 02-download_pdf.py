...
mapply.init(n_workers=32, chunk_size=50, max_chunks_per_worker=0, progressbar=True)
    ...
    while retry < 5:
        try:
            ...
            if text != 0:
                ...
                captcha = {"code": "", "error_code": solver.error_code}
            with open(cfn.replace("jpg", "json"), "w") as f:
                f.write(json.dumps(captcha))
            data = {
                "__VIEWSTATE": viewstate,
...
def download(r):
    ...
    fn = os.path.join(
        OUTPUT_DIR, "%s_%d_%d.pdf" % (r["District"], r["AC_Number"], r["Part_Number"])
    )
...
df = pd.read_csv(
    "data/up-2023-electoral-rolls.csv",
    usecols=["District", "AC_Number", "Part_Number", "ElectorRollPdf"],
)
print(df)
